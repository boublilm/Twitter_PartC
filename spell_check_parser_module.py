    ...
    def parse_sentence(self, text):
        ...
        for term in text_tokens:
            ...
            if term[0] == "#":  # handle hashtags
                ...
                indices_counter -= 1
                continue
            term = self.checker.correction(term)
    ...
    def remove_stopwords(self,query):
        ...
        for term in tokens:
            if term.lower() in self.corona_words:
                ...
                lower = term.islower()
                if self.toStem:
                    new_term = self.stemmer.stem_term(self.checker.correction(term))
                else:
                    new_term = self.checker.correction(term)
                if lower:
                    real_ones.append(new_term.lower())
                else:
